...
### `console` - a feed of tone-dotted log lines
```json
...
Each line shows a dot in the color of its `tone`, the `status` word, and the `text`. The optional `meta` shows at the right. `tone` uses the palette above. `text` is the only required line field.

`ts` is a UTC ISO-8601 instant (`…Z`). The renderer changes it to the viewer's time zone and puts it before `meta`. **A collector must write `ts`, and never a formatted local or UTC string.** A formatted string shows the clock of the collector machine to every viewer.

These line fields are optional:

- `href` makes the line text a link, for example to the Actions run.
- `cmd` adds a copy-to-clipboard chip after the text, for example a `gh run watch` line.
- `logo` marks the line with a stack (`swift | ts | js`), so one merged feed can show whose each run is.
- `pill` gives the status word when `status` is absent. The renderer reads `pill.text`.

The renderer also accepts three old field names: `items` for `lines`, `q` for `text`, and `note` for `meta`.

`scroll` (optional, a row count) limits the block to that many rows and scrolls the other rows. A console can then hold a long record and not push every section below it off the page. Without `scroll`, the block shows at full height. A console that scrolls must put its `cmd` chips first, because rows after the fold are hard to find. A console with no lines shows `no active runs`.

A console can also link its heading to a detail page with `href`, like any titled section:
...
The History boards, the CI runs section, and the `Builds`, `Stacks`, `Seats`, and `Collectors` sections are consoles.

### `live-console` - a console that refreshes itself in the browser
```json
{ "kind": "live-console", "icon": "🔴", "title": "CI — running now",
  "desc": "live queued/in-progress runs",
  "poll": { "url": "https://ci.example.net/api/runs?project=phoenix", "intervalMs": 30000 } }
```

A `live-console` has no `lines` in `board.json`. The browser gets the rows from `poll.url` at run time, so the section can show runs that start between two deploys. `poll` is required, and `bin/validate-board.py` fails a board whose `poll` has no `url`.

The endpoint returns `{ "lines": [...], "intervalMs": 20000, "updatedAt": "…" }`. Each line has the same shape as a `console` line. `intervalMs` and `updatedAt` are optional.

- The renderer gets `poll.url` one time when the page loads, and then again at each interval.
- The first interval is `poll.intervalMs` (default 30000). When a response has `intervalMs`, the renderer uses that value from then on. The endpoint sets the refresh rate, and the board follows it.
- Under the heading, a status row shows a live dot and the time of the last good fetch. If a fetch fails, the dot goes stale and the row says `unreachable — retrying`. With no `poll.url`, the row says `no endpoint`.

A `live-console` does not use `scroll`, `collapsible`, or `collapsed`. `collect/ci_live.py` writes this section, and [INTERFACES.md](INTERFACES.md) gives the relay behind the endpoint.
...
The [README Collectors table](README.md#collectors) lists every collector under `bin/collect/` and the sections each one writes.
